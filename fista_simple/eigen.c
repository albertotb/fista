...
*******************************************************************************/
{
   ...
   for (i=n;i>=2;i--) {
      ...
      if (l > 1) {
         ...
         if (scale == 0.0) /* Skip transformation. */
            ...
            for (j=1;j<=l;j++) {
               ...
               g=0.0; /* Form an element of A \dot u in g. */
               for (k=1;k<=j;k++)
                  g += a[j][k]*a[i][k];
               for (k=j+1;k<=l;k++)
                  g += a[k][j]*a[i][k];
      ...
      if (d[i]) { /* This block skipped when i=1. */
         for (j=1;j<=l;j++) {
            g=0.0;
            for (k=1;k<=l;k++) /* Use u and u/H stored in a to form P \dot Q. */
               g += a[i][k]*a[k][j];
            for (k=1;k<=l;k++)
               a[k][j] -= g*a[k][i];
         }
 ...
 * ***************************************************************************/
double max_eig(double **X, int n)
{
   double max = DBL_MIN;
   double **A = convert_matrix(&X[0][0], 1, n, 1, n);
